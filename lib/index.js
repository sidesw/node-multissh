...
var MultiSsh = {
  ...
  init: function(args) {
    ...
    var idx = myArgv.indexOf('-u');
    if(idx > -1 && idx < myArgv.length - 1) {
      MultiSsh.userName = myArgv[idx+1];
      myArgv.splice(idx, 2);
    }

    idx = myArgv.indexOf('-p');
    if(idx > -1 && idx < myArgv.length - 1) {
      var port = parseInt(myArgv[idx+1], 0);
      if(isFinite(port)) {
        MultiSsh.port = port;
      }
      myArgv.splice(idx, 2);
    ...
    if(myArgv.length === 0) {
      console.log("usage: multissh [-u user] [-p port] tom dick harry [ -c cmd ]");
      return;
    }
  ...
  start: function() {
    ...
    ui.on('SIGINT', function() {
      ui.question('Do you want to send an interrupt to all servers? ', function(answer) {
        if(answer.match(/^y(es)?$/i)) {
          MultiSsh.servers.forEach(function(server) {
            MultiSsh.connections[server].kill('SIGINT');
          });
        }
      });
    });
  ...
  connect: function(callback) {
    ...
    MultiSsh.servers.forEach(function(server) {
      var args = [];
      if(MultiSsh.userName) {
        args.push(MultiSsh.userName + '@' + server);
      } else {
        args.push(server);
      }
      if(MultiSsh.port) {
        args.push('-p');
        args.push(MultiSsh.port.toString());
      }
